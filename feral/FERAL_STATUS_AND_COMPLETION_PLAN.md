...
- Save-compatible v13 MCM migration plus the existing mastery, visual, milestone, human-response, passive, and duration migrations.
...
- A separate hash-pinned Sex Grants Experience 1.8.0 integration with shape-gated XP, matching-family scene mastery, temporary level-10 matching-creature neutrality, and consensual arousal/time-driven SexLab approaches beginning after five seconds.
...
## Confirmed in game before v13

- The Feral MCM registers and opens.
- Player-kill detection and automatic Skeever harvesting work. The v13 page repair, full progression display, scene/kinship integration, human response, and long-duration visual cleanup still need confirmation.

## Remaining validation

- Run the consolidated v13 in-game playtest in `FERAL_FINAL_PLAYTEST.md`, including neutrality restoration, broken kinship, approach timing, refusal, cooldown, and mid-scene shape expiration.
...
- Add fully conditioned NPC dialogue and purpose-built hunter NPCs after the event-driven response framework is proven. v13 currently uses threshold notifications and vanilla bandit templates.
...
- v13's hot-path rule is explicit: kill handling rejects irrelevant events early; active shapes and techniques never register updates; visual growth and morph overrides are calculated only on cast; human response uses bounded event-time queries only; integration detection runs only while building the MCM. The optional kinship controller scans only while it has a qualifying shape or accepted scene to manage.
